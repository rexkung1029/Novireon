...
class PingCommands:
    ...
    def get_latency_color(self, latency_ms: float) -> discord.Color:
        if latency_ms < 80: return discord.Color.green()
        elif latency_ms < 150: return discord.Color.yellow()
        else: return discord.Color.red()
    ...
    async def ping(self, itat: discord.Interaction):
        ...
        uptime = self.format_uptime(datetime.datetime.utcnow() - self.bot.start_time) if hasattr(self.bot, 'start_time') else "N/A"
        ...
        embed.set_footer(text=f"由 {itat.user.display_name} 請求", icon_url=itat.user.avatar.url if itat.user.avatar else None)
